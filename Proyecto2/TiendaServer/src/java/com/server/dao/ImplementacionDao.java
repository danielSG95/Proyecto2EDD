...
//        buffer.append("graph[rankdir=LR, ordering=out];\n");
    ...
    public void reporteTopUsuarios(String fechaI, String fechaF) {
        ...
        LocalDate fechai = LocalDate.parse(fechaI, formatter2);
        LocalDate fechaf = LocalDate.parse(fechaF, formatter2);
        
        top = ARBOLB.reporteTop(fechai, fechaf);
        top.ordenar(top);
        if(top.graficar("BusquedaPorRango")) {
            try {
                    Runtime rt = Runtime.getRuntime();
                    rt.exec("ristretto " + RUTAIMG + "BusquedaPorRango.png");
                } catch (Exception e) {
                    System.out.println(e.toString());
                }
        }
    ...
    public String reporteBusquedaPorFecha(String fecha) {
        ...
        if(report.getPrimero() != null) { //si se lleno           
           if (report.graficar("BusquedaPorFecha")) {
                try {
                    Runtime rt = Runtime.getRuntime();
                    rt.exec("ristretto " + RUTAIMG + "BusquedaPorFecha.png");
                } catch (Exception e) {
                    System.out.println(e.toString());
                }
            }
    ...
    private String  generarFactura(String codigoFactura, String nickname) {        
        ...
        if(factura != null) {            
            ...
            if(temporal != null) { //cambiar por temporal != null
    ...
    public String listaDeFacturas(String username) {
        StringBuffer buffer = new StringBuffer();
        ListaSimple facturas = ARBOLB.facturasPorNick(username);
        if(!facturas.esVacio()) {            
            NodoSimple pivote = facturas.getPrimero();
            while(pivote != null) {
                Venta v = (Venta)pivote.getDato();
                buffer.append(v.getNumeroFactura()).append("\n");
                pivote = pivote.getSiguiente();
            }
        }
        return buffer.toString();
